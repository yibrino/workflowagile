  ...
  for _, row in df.iterrows():
      ...
      for column_name in selected_columns:
          if column_name == "MultipleChoiceOptions":
              ...
              answers = set(map(int, str(row["MultipleChoiceAnswers"]).split(';'))) if pd.notnull(row["MultipleChoiceAnswers"]) else set()

              answer_options = [{"text": option.strip(), "isCorrect": index + 1 in answers} for index, option in enumerate(options)]
